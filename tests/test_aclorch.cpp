...
void assign_default_acltable_attr(vector<sai_attribute_t> &table_attrs)
{
    ...
    memset(&attr, 0, sizeof(attr));
    range_types_list = { SAI_ACL_RANGE_TYPE_L4_DST_PORT_RANGE, SAI_ACL_RANGE_TYPE_L4_SRC_PORT_RANGE };
...
bool verify_acltable_attr(vector<sai_attribute_t> &expect_table_attrs, sai_attribute_t *verify_attr_p) {
        ...
        if (it.id == verify_attr_p->id) {
            switch (it.id) {
                ...
                case SAI_ACL_TABLE_ATTR_ACL_BIND_POINT_TYPE_LIST:
                    if ((it.value.s32list.count == verify_attr_p->value.s32list.count) &&
                        0 == memcmp(it.value.s32list.list, verify_attr_p->value.s32list.list, sizeof(it.value.s32list.list[0])*it.value.s32list.count))
                        return true;
